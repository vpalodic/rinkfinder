...
//		$cs->registerScriptFile($baseUrl . '/js/form.js');
		...
		$js = "
				...
				buttons: { '" . UserModule::t('Save')."': function()
					{
						...
						{
							if($(this).val()) {
									wparam[$(this).attr('name')] = $(this).val();
							}
						});
						...
						{
							if($(this).val()) {
								fparam[tab][$(this).attr('name')] = $(this).val();
							}
						});

						if($.JSON.encode(wparam) != '{}') {
							$('div.widgetparams input').val($.JSON.encode(wparam));
						}

						if($.JSON.encode(fparam[tab]) != '{}') {
							$('div.other_validator input').val($.JSON.encode(fparam));
						}
			...
			$('#widgetparams').focus(function()
			{
				...
				var html = '';
				var wparam = ($('div.widgetparams input').val()) ?
								$.JSON.decode($('div.widgetparams input').val()) :
								{};
				var fparam = ($('div.other_validator input').val()) ?
								$.JSON.decode($('div.other_validator input').val()) :
								{};
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['ProfileField']))
		{
			...
			if($model->validate()) {
				...
				if($model->field_type != 'TEXT' && $model->field_type != 'DATE' && $model->field_type != 'BOOL'	&&
				   $model->field_type != 'BLOB' && $model->field_type != 'BINARY') {
				...
				if($model->field_type != 'TEXT' && $model->field_type != 'BLOB' || $scheme != 'CMysqlSchema') {
					if($model->default) {
						...
						$sql .= (($model->field_type == 'TEXT' || $model->field_type == 'VARCHAR' ||
							  $model->field_type == 'BLOB' || $model->field_type=='BINARY') ?
							 " DEFAULT ''" :
	...
	public function actionDelete()
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			if($scheme == 'CSqliteSchema') {
				...
				{
					$sql = '';
					$connection->createCommand("CREATE TEMPORARY TABLE " .
					...
					$connection->createCommand("INSERT INTO " .
								   ...
								   implode(',', $attr) .
								   " FROM " .
								   Profile::model()->tableName())->execute();

					$connection->createCommand("DROP TABLE " .
								   Profile::model()->tableName())->execute();

					$connection->createCommand("CREATE TABLE " .
								   Profile::model()->tableName() .
								   " (" .
								   implode(',', $attr) .
								   ")")->execute();
