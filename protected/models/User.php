...
//                'select' => 'id, username, email, password, failed_logins, status_id, lock_version',
    ...
    public static function getAvailableManagers($uid, $aid)
    {
        $sql = 'SELECT u.*, p.* '
                ...
                . 'INNER JOIN (SELECT DISTINCT aua.user_id '
                ...
                . '             UNION '
                . '             SELECT u.id FROM user u where u.created_by_id = :uid) u1 '
                . 'ON u1.user_id = u.id '
                . 'AND u.id NOT IN (SELECT DISTINCT aua.user_id '
                . '                 FROM arena_user_assignment aua '
                . '                 WHERE aua.arena_id IN (SELECT DISTINCT aua1.arena_id '
                . '                         FROM arena_user_assignment aua1 '
     ...
     * @throws CDbException
     */
    public static function getAvailableManagersForAdmins($uid, $aid)
    {
        $sql = 'SELECT u.*, p.* '
                . 'FROM user u '
                . 'INNER JOIN profile p '
